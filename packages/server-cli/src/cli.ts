...
export function parseBootstrapRequest(args: string[], host?: HostAdapter): BootstrapRequest {
  ...
  const request: BootstrapRequest = { command, nonInteractive: false, approve: false, manageFirewall: false, dedicatedServiceAccounts: false };
...
export async function runBootstrap(args: string[], options: RunBootstrapOptions): Promise<BootstrapResult> {
  ...
  const state = await commitOwnedState(prepared, options.state);
  const handoff = createCredentialHandoff(credentials);
  if (request.nonInteractive) await handoff.writeUnattended(request.secretsOutput!, options.secretOutput!);
  else await handoff.discloseInteractive(options.discloseInteractiveSecrets!);
...
export async function runVaultCommand(args: string[], options: RunVaultCommandOptions): Promise<VaultCommandResult> {
  ...
  if (action === "revoke") { await revokeVaultUser(adapter, administrator, target); return undefined; }
  ...
  const handoff = vaultSecretHandoff(result.credential.username, result.credential.password);
  if (input) await options.secretOutput!.writeFileAtomically(output!, handoff, { owner: 0, mode: 0o600 });
