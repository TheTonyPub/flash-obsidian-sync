...
describe("plugin diagnostics", () => {
  ...
  it("always logs errors and enables technical events only in debug mode", () => {
    ...
    expect(sink.error).toHaveBeenCalledWith("[easy-sync] nats.connect: Authorization Violation", { bucket: "OBS_A_FILES" });
    debug = true;
    logger.debug("nats.connected", { bucket: "OBS_A_FILES" });
    expect(sink.debug).toHaveBeenCalledWith("[easy-sync] nats.connected", { bucket: "OBS_A_FILES" });
  });
  ...
  it("logs a background vault read failure without an unhandled rejection", async () => {
    ...
    expect(sink.error).toHaveBeenCalledWith("[easy-sync] vault.read_failed: read failed", {});
    engine.stop();
    store.close();
  });
});
