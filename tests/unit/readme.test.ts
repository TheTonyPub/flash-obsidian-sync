...
describe("README", () => {
  it("documents source installation, prerequisites, and server connection", async () => {
    const readme = await readFile(readmePath, "utf8");

    expect(readme).toContain("Manual installation from source");
    expect(readme).toContain("Prerequisites");
    expect(readme).toContain("npm ci");
    expect(readme).toContain("npm run build:plugin");
    expect(readme).toContain(".obsidian/plugins/flash-osidian-sync");
